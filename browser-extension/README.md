# CyberAid Guardian (Browser Extension)

Real-time phishing & scam protection for citizens. The extension contains **no
AI/model logic** — every check calls the existing CyberAid FastAPI backend.
...
- **Right-click analysis** — "Analyze with CyberAid" on any link/page runs `POST /api/detect` (full result + Gemini AI summary).
...
- **One-click reporting** — files a complaint via `POST /api/complaints` (reuses your CyberAid login).
...
| Auth | JWT bridged from the CyberAid web session |
...
4. Pin CyberAid Guardian to the toolbar.
5. Log in to the CyberAid citizen portal once so the extension can report on your behalf.

## Security & privacy
- No passwords are ever stored. The extension reuses the JWT from your active CyberAid web session for reporting only.
